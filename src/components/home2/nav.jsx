...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#F2F1EF] shadow-md' : 'bg-[#F2F1EF]'
      }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden md:flex md:items-center md:space-x-8">
            {["Home", "Services", "About Us", "Contact Us"].map((item, i) => (
              <Link key={i}
                ...
                className={`text-[#723E31] px-3 py-2 rounded-md text-sm font-medium transition-all hover:-translate-y-0.5 hover:bg-[#D9D2CC]/60 ${location.pathname === `/${item.toLowerCase().replace(" ", "")}` ? 'bg-[#D9D2CC]/80' : ''
                  }`}
              >
                {item}
              </Link>
          ...
          <div className="hidden md:block">
            <a href="#booking" className="bg-[#D9B18E] text-[#F2F1EF] px-6 py-2 rounded-md font-semibold transition-all hover:-translate-y-0.5 hover:shadow-lg hover:bg-[#A67564]">
              Book Appointment
            </a>
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-[#F2F1EF] shadow-lg px-4 py-4 space-y-2">
          ...
          <a href="#booking" className="block text-center w-full bg-[#D9B18E] text-[#F2F1EF] px-6 py-2 rounded-md font-semibold hover:bg-[#A67564] transition-all">
            Book Appointment
          </a>
        </div>
      )}
